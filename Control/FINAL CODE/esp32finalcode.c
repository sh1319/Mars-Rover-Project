...
String batterylevel = "100"; //example value
String dist = "37.8"; //example value
String tempdriveinstr; //made to string for server, will have to Stoi on Drive side.
...
void serverconnect(){
 ...
 void serverhandshake()
 {
    ...
    String roverstatus = batterylevel;
...
void batterycheck() //Energy - ESP32 - Command. Used to update GUI of battery level of the rover
{
  ...
  {
  batterylevel = Serial2.readString();
...
void receivedrivedist() //get distance travelled from arduino
{
  ...
  dist = Serial1.readString(); //do I require to decode this info? also if 8bit count am I limited to 2^8?, will have to stoi on other side
...
void senddriveinstr() //where n comes from receivenewdriveinstr, sends to Drive Arduino
{
  if(Serial1.available() > 0)
  {
  Serial1.write(newdriveinstr); //is this required because base value of int is 16 bits long
  }
  else
  {
